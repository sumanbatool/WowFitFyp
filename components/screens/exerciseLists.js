...
//   //  const radioValue = { exerciseName: selectedExercise };
//     // const exercisesToSave = selectedExercise.map(exerciseName => {
//     //   const exercise = exercises.find(exercise => exercise.name === exerciseName);
//     //   return { exerciseName, iconUrl: exercise.icon };
//     // });
//     const exercisesToSave = selectedExercise.map((exercise) => {
//       return { exerciseName: exercise.name, icon: exercise.icon };
//     });
//     const exerciseNames = selectedExercise.map((exercise) => exercise.name);
//   const requestData = {
...
//       });
//       navigation.navigate('NewWorkout', { selectedExercise:[exercisesToSave]});
//   };
//   const renderExerciseItem = ({ item }) => {
//     // const isChecked = selectedExercise.includes(item.name);
//     const isChecked = selectedExercise.includes(item);
//     return (
...
const ExerciseScreen = (props) => {
  ...
  const renderExerciseItem = ({ item }) => {
    const isChecked = selectedExercise.some((exercise) => exercise.name === item.name);
    return (
      <Pressable onPress={() => handleRadioButtonPress(item)}>
        <View style={Liststyles.itemContainer}>
          <Image source={{ uri: item.icon }} style={{ width: 50, height: 50 }} />
